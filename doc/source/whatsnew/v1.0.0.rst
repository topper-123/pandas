...
- The level codes in :attr:`pandas.MultiIndex.codes` were previously ``pandas.core.indexes.frozen.FrozenNDArray``,
  which were previously deprecated and have now been removed from the Pandas code base. The codes are now numpy arrays with the writeable flag set to False (:issue:`xxxx`).
...
- Removed ``pandas.core.indexes.frozen.FrozenNDArray`` (:issue:`xxxxx`).
